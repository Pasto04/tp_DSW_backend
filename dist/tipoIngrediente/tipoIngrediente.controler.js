...
async function findAll(req, res) {
  try {
    ...
    res
      .status(200)
      .json({
        message: 'Todos los tipos de ingrediente fueron encontrados',
        data: tiposIngre,
      });
  } catch (error) {
...
async function findOne(req, res) {
  try {
    ...
    const tipoIngre = await em.findOneOrFail(TipoIngrediente, {
      codigo: codTipoIngre,
    });
    res
      .status(200)
      .json({
        message: 'El tipo de ingrediente fue encontrado con éxito',
        data: tipoIngre,
      });
  } catch (error) {
    res.status(500).json({ message: error.message });
  ...
  try {
    ...
    await em.flush(),
      res
        .status(201)
        .json({
          message: 'El tipo de ingrediente fue creado con éxito',
          data: tipoIngre,
        });
  } catch (error) {
    res.status(500).json({ message: error.message });
...
async function update(req, res) {
  try {
    ...
    res
      .status(200)
      .json({
        message: 'El tipo de ingrediente fue actualizado con éxito',
        data: tipoIngre,
      });
  } catch (error) {
    res.status(500).json({ message: error.message });
...
async function remove(req, res) {
  try {
    const codigo = Number.parseInt(req.params.cod);
    const deletedTipoIngre = await em.findOneOrFail(TipoIngrediente, {
      codigo,
    });
    ...
    res
      .status(200)
      .json({
        message: 'El tipo de ingrediente ha sido eliminado con éxito',
        data: deletedTipoIngre,
      });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
