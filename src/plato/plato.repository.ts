...
const platos=[
  new Plato(
    'b780c4c6-68c2-4177-8371-af0102a99d39',
...
export class PlatoRepository implements Repository<Plato>{
  ...
  public findOne(item: { nro: string; }): Plato | undefined {
    return platos.find((plato) =>plato.nro===item.nro)
  }
  ...
  public delete(item: { id: string; }): Plato | undefined {
    const platoIdx = platos.findIndex((plato) =>plato.nro===item.id)
    if(platoIdx!==-1){
