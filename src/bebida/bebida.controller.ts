...
import {
  BebidaHasNoProveedor,
  BebidaNotFoundError,
  BebidaPreconditionFailed,
  BebidaUniqueConstraintViolation,
} from '../shared/errors/entityErrors/bebida.errors.js';
...
async function findAll(req: Request, res: Response) {
  try {
    ...
    if (descripcionParcial) {
      bebidas = validarFindAll(
        await em.find(Bebida, {
          descripcion: { $like: `%${descripcionParcial}%` },
        }),
        BebidaNotFoundError
      );
    } else {
    ...
    res
      .status(200)
      .json({
        message: `Todas las bebidas han sido encontradas con éxito`,
        data: bebidas,
      });
  } catch (error: any) {
...
async function findOne(req: Request, res: Response) {
  try {
    ...
    res
      .status(200)
      .json({
        message: `La bebida ${bebida.descripcion} ha sido encontrada con éxito`,
        data: bebida,
      });
  } catch (error: any) {
    handleErrors(error, res);
...
async function add(req: Request, res: Response) {
  try {
    if ((await em.find(Proveedor, {})).length === 0) {
      ...
      const proveedor = await em.findOneOrFail(
        Proveedor,
        { id },
        {
          failHandler: () => {
            throw new ProveedorNotFoundError();
          },
        }
      );
      const bebidaDeProveedor = em.create(BebidaDeProveedor, {
        bebida,
        proveedor,
      });
      // creación de la relación entre bebida y proveedor
...
async function update(req: Request, res: Response) {
  try {
    ...
    res
      .status(200)
      .json({
        message: `La bebida ${bebida.descripcion} fue actualizada con éxito`,
        data: bebida,
      });
  } catch (error: any) {
    if (error.name === 'UniqueConstraintViolationException') {
...
async function remove(req: Request, res: Response) {
  try {
    ...
    res
      .status(200)
      .json({
        message: `La bebida ${bebida.descripcion} ha sido eliminada con éxito`,
        data: bebida,
      });
  } catch (error: any) {
    handleErrors(error, res);
