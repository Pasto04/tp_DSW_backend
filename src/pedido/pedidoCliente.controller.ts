...
import {
  ...
  PedidoAlreadyExistsError,
  PedidoAlreadyInUseError,
  PedidoNotFoundError,
  PedidoPreconditionFailed,
  PedidoUniqueConstraintViolationError,
} from '../shared/errors/entityErrors/pedido.errors.js';
import {
  validarPedido,
  validarPedidoCancelar,
  validarPedidoFinalizar,
} from './pedido.schema.js';
...
import {
  ...
  MesaCodigoError,
  MesaNotFoundError,
} from '../shared/errors/entityErrors/mesa.errors.js';
...
async function sanitizePedidoCliente(
  req: Request,
  res: Response,
  next: NextFunction
) {
...
async function findAll(req: Request, res: Response) {
  try {
    ...
    const pedidos = validarFindAll(
      await em.find(Pedido, sanitizedQuery),
      PedidoNotFoundError
    );
    res.status(200).json({
      message: `Todos los pedidos del cliente ${cliente.nombre} ${cliente.apellido} encontrados con éxito`,
      data: pedidos,
    });
  } catch (error: any) {
...
async function findOne(req: Request, res: Response) {
  try {
    ...
    const pedido = await em.findOneOrFail(
      ...
      { nroPed },
      {
        populate: ['platosPedido', 'bebidasPedido', 'cliente'],
        failHandler: () => {
    ...
    res.status(200).json({
      message: `Pedido ${nroPed} del cliente ${pedido.cliente.nombre} ${pedido.cliente.apellido} encontrado`,
      data: pedido,
    });
  } catch (error: any) {
    handleErrors(error, res);
...
async function add(req: Request, res: Response) {
  try {
    if (
      (await em.find(Usuario, { tipoUsuario: 'cliente' })).length === 0 ||
      (await em.find(Mesa, {})).length === 0
    ) {
    ...
    req.body.sanitizedInput.mesa = await em.findOneOrFail(
      Mesa,
      { nroMesa: req.body.sanitizedInput.mesa },
      {
        failHandler: () => {
...
async function update(req: Request, res: Response) {
  try {
    const nroPed = Number.parseInt(req.params.nroPed);
    const pedidoToUpdate = await em.findOneOrFail(
      Pedido,
      { nroPed },
      {
        populate: ['platosPedido', 'bebidasPedido', 'cliente'],
    ...
    if (
      ...
      pedidoToUpdate.estado === 'cancelado'
    ) {
      throw new PedidoAlreadyEndedError();
    }
    fillSanitizedInput(pedidoToUpdate, req);
    req.body.sanitizedInput.pago = await em.findOneOrFail(
      Pago,
...
async function cancel(req: Request, res: Response) {
  try {
    ...
    if (
      ...
      pedidoToUpdate.platosPedido.length > 0 ||
      pedidoToUpdate.bebidasPedido.length > 0
    ) {
